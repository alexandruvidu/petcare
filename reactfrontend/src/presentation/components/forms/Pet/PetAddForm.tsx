...
import { usePetAddFormController } from "./PetAddForm.controller"
import { ContentCard } from "@presentation/components/ui/ContentCard"
import { isEmpty, isUndefined } from "lodash"

/**
 * Pet add form component
 */
export const PetAddForm = ({ onSuccess }: { onSuccess?: () => void }) => {
  const { formatMessage } = useIntl()
  const { state, actions, computed } = usePetAddFormController(onSuccess)

  return (
    <form onSubmit={actions.handleSubmit(actions.submit)}>
      <Stack spacing={4} style={{ width: "100%" }}>
        <ContentCard title={formatMessage({ id: "globals.add" }) + " " + formatMessage({ id: "globals.pet" })}>
