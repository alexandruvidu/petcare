...
import type { UserFileAddFormController, UserFileAddFormModel } from "./UserFileAddForm.types"
...
const useInitUserFileAddForm = () => {
  ...
  const schema = yup.object().shape({
    file: yup
      .mixed<File>() // For files the schema used should be mixed.
      .required(
        formatMessage(
...
export const useUserFileAddFormController = (onSubmit?: () => void): UserFileAddFormController => {
  ...
  const submit = useCallback(
    (
      data: UserFileAddFormModel, // Create a submit callback to send the form data to the backend.
    ) =>
      add(data).then(() => {
        if (onSubmit) {
          onSubmit()
  ...
  const setFile = useCallback(
    (file: File) => {
      // The file will be added via a button so create a callback to set the file value.
      setValue("file", file, {
        shouldValidate: true,
      })
    },
