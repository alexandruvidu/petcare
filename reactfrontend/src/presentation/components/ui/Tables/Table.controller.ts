...
export const useTableController = (
  ...
  const handleChangePage = useCallback(
    (
      // Create a callback to listen on changes of the table page.
  ...
  const handleChangePageSize = useCallback(
    (
      // Create a callback to listen on changes of the table page size.
      event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>,
    ) => {
      onPaginationChange(1, Number.parseInt(event.target.value, 10)) // Reset the current page to 1 on page size change to avoid overflow.
    },
    [onPaginationChange],
  )

  const labelDisplay = useCallback(
    ({ to, from, count }: LabelDisplayedRowsArgs) => {
      // Create a callback to display the paging labels with translations.
      return count !== -1
