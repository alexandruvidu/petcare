...
const useHeader = (): DataTableHeader<UserFileDTO> => {
  ...
  return [
    ...
    {
      key: "user",
      name: formatMessage({ id: "globals.addBy" }),
      order: 3,
      render: (value) => <span>{value.name}</span>,
    },
    {
      key: "createdAt",
      name: formatMessage({ id: "globals.createdAt" }),
      order: 4,
      render: (value) => <span>{dateToDateStringOrNull(value)}</span>,
    },
  ]
}
...
export const UserFileTable = () => {
  ...
  const {
    ...
    downloadUserFile,
    openUserFile,
  } = useUserFileTableController() // Use the controller hook.
            ...
            render: (entry) => (
              <>
                ...
                {entry.name?.endsWith(".pdf") && (
                  <IconButton color="primary" onClick={() => openUserFile(entry)}>
