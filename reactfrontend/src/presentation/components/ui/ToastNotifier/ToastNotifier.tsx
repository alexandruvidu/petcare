  ...
  useInterceptor({
    async onResponse(response: Response) {
      if (response.status === 401 && tokenHasExpired.loggedIn && tokenHasExpired.hasExpired) {
        // If the token has expired show the proper notification.
        ...
        // If the server returns an error with a error code show the proper notification with a message corresponding to the error code.
