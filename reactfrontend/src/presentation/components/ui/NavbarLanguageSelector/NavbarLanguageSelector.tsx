...
import { useLanguageAPI, useLanguageState } from "application/context/LanguageContextProvider"
    ...
    <div>
      ...
      >
        <ul className="language__selector__items">
          <li
            onClick={setRomanian}
            className={`language__selector__item ${appLanguage === SupportedLanguage.RO ? "active" : ""}`.trim()}
          >
            <FormattedMessage id="nav.languageSelector.ro" />
          </li>
          <li
            onClick={setEnglish}
            className={`language__selector__item ${appLanguage === SupportedLanguage.EN ? "active" : ""}`.trim()}
          >
