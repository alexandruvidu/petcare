...
export const profileSlice = createSlice({
  ...
  reducers: {
    setToken: (_, action: PayloadAction<string>) => {
      // The payload is a wrapper to encapsulate the data sent via dispatch. Here the token is received, saved and a new state is created.
    ...
    resetProfile: () => {
      // This removes the token from the storage and resets the state.
      localStorage.removeItem(tokenKey)
...
export const { setToken, resetProfile } = profileSlice.actions // Export the slice actions, they are used to wrap the data that is send via the dispatch function to the reducer.
